...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    # gtboxs = overlaps.argmax(axis=1)
    anchor_argmax_overlaps = overlaps.argmax(axis=1)

    print("anchor_argmax_overlaps", anchor_argmax_overlaps)

    #print("anchor_argmax_overlaps", anchor_argmax_overlaps.shape)
    anchor_max_overlaps = overlaps[range(overlaps.shape[0]), anchor_argmax_overlaps]
    #print("anchor_max_overlaps", anchor_max_overlaps.shape)

    # IOU > IOU_POSITIVE
    print(np.where(anchor_max_overlaps > IOU_POSITIVE))
