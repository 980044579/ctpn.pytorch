...
class RPN_REGR_Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        a = less_one * 0.5 * diff ** 2 * self.sigma
        b = torch.abs(1- less_one) * (diff - 0.5/self.sigma)
...
class RPN_CLS_Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # loss = F.sparse_softmax_cross_entropy_with_logits
        lossF = torch.nn.CrossEntropyLoss(ignore_index=-1)
        loss =lossF(cls_pred, cls_true)    
        # loss = F.nll_loss(cls_pred, cls_true)  # original is sparse_softmax_cross_entropy_with_logits
...
if __name__ == '__main__':
    ...
    img = torch.rand((3, 3, 720, 1280))
    model = CTPN_Model()
    clsp, regrp = model(img)
    ...
    regrp = regrp.double()
    regr = regr.double()
    loss = RPN_REGR_Loss("cpu")
    print(regr.shape)
    print(regrp.shape)
    print(loss(regrp, regr))
